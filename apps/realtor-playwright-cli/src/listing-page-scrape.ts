/**
 * Browser-only: mirrors `apps/chrome-extension/src/content/content-script.ts` (buildScrapeSource)
 * without the Chrome messaging bridge. Used by the IIFE bundle for Playwright.
 */
import type { ScrapeSource } from "../../chrome-extension/src/content/scraper.js";
import { parseListingPayload } from "../../chrome-extension/src/content/scraper.js";
...
/** Best-effort lot / land line for rawSnapshot (extension does not model this field). */
export function extractLandSizeHint(bodyText: string | undefined): string | undefined {
  if (!bodyText) return undefined;
  const lines = bodyText.split(/\n/);
  for (const raw of lines) {
    const line = raw.replace(/\s+/g, " ").trim();
    if (!line) continue;
    if (/lot\s*(size|area)|land\s*(size|area)|lot\s+frontage|acre(s)?\b/iu.test(line)) {
...
export function buildScrapeSource(): ScrapeSource {
...
export function scrapeListingPagePayload(): ReturnType<typeof parseListingPayload> {
  const source = buildScrapeSource();
  const payload = parseListingPayload(source);
  const landHint = extractLandSizeHint(source.bodyText);
  if (landHint) {
    payload.rawSnapshot = {
      ...payload.rawSnapshot,
      landSizeHint: landHint
    };
  }
  return payload;
}
